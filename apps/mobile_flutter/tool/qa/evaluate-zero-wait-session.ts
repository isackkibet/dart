...
import type { ZeroWaitSwipeResult } from "./zero-wait-swipe-result.ts";
...
function evaluate(
  ...
  const cacheHitCount = results.filter(
    (r) => r.cacheSource === "hot-controller" || r.cacheSource === "disk-cache",
  ).length;
  ...
  if (maximumControllerCount > thresholds.maximumControllers) {
    failures.push(`Controller count reached ${maximumControllerCount}.`);
  }

  for (const result of results) {
    if (result.status === "FAIL") {
      failures.push(
        `Swipe ${result.swipeNumber} failed${result.defectId ? ` (${result.defectId})` : ""}.`,
      );
    }
  }
