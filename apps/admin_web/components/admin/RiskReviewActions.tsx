...
export function RiskReviewActions({ reviewId }: { reviewId: string }) {
  ...
  async function act(action: 'approve' | 'escalate') {
    setBusy(true);
    setMessage('');
    try {
      const res = await fetch(`/api/admin/risk-reviews/${reviewId}/${action}`, {
        method: 'POST',
