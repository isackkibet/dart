...
exports.rewardedAdProcessor = void 0;
...
async function hasElapsedSinceViewStart(userId, campaignId, requiredSeconds) {
    const viewStartSnap = await db
        ...
        .limit(1)
        .get();
    if (viewStartSnap.empty)
        return false;
    const viewStartAt = viewStartSnap.docs[0].data().createdAt;
    if (!viewStartAt)
        return false;
...
exports.rewardedAdProcessor = (0, firestore_1.onDocumentCreated)({
    ...
    const { userId, campaignId, type } = data;
