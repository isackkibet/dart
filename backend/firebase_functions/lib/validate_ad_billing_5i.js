...
async function waitForProcessing(collection, docId, maxWaitMs = 15000) {
...
async function waitForCampaignSpend(campaignId, expectedSpend, maxWaitMs = 15000) {
    const start = Date.now();
    while (Date.now() - start < maxWaitMs) {
        const snap = await db.collection('adCampaigns').doc(campaignId).get();
        const data = snap.data();
        if (data && Number(data.spend ?? 0) >= expectedSpend)
            return data;
        await new Promise((r) => setTimeout(r, 1000));
    }
    return null;
...
async function main() {
    ...
    await db.collection('adEngagementEvents').doc(DUPLICATE_EVENT_ID).set({ updatedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
    await new Promise((r) => setTimeout(r, 6000));
    const spendAfterDuplicate = Number((await db.collection('adCampaigns').doc(IDEM_CAMPAIGN_ID).get()).data()?.spend ?? 0);
